...
const handleClick = ()=>{
    ...
    axios.post('http://165.227.119.71/api/auth/login/', params)
        .then(response => {console.log(response);localStorage.setItem('token',response.data.token);navigation.navigate('TabNavigation')})
        .catch((err)=>{Alert.alert("INCORRECT CREDENTIALS");navigation.navigate('Login')}); 
};
